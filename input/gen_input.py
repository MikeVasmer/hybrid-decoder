...
# uPs = [0.2, 0.4, 0.6, 0.8]
uPs = [1]
ePs = [0.05, 0.06, 0.07, 0.08, 0.09, 0.1, 0.11]
# ePs = [0.078, 0.082, 0.084, 0.086, 0.088, 0.092]
# ePs = { \
...
trials = 1e4
job = 0
disorder = {0 : 1, 0.2 : 100, 0.4 : 100, 0.6 : 100, 0.8 : 100, 1 : 100}
git_hash = subprocess.check_output(['git', 'rev-parse',  'HEAD']).strip()
randomize_unencoding = True

with open('14_01_21a.csv', 'w') as csv_file:
    writer = csv.writer(csv_file, delimiter=',')
    writer.writerow(['L', 'uP', 'eP', 'trials', 'disorder_avg', 'randomize_unencoding', 'job_number', 'git hash (metadata)'])
    for uP in uPs:
        for L in Ls:
            for eP in ePs:
                writer.writerow([L, uP, eP, trials, disorder[uP], randomize_unencoding, job, git_hash])
                job += 1
